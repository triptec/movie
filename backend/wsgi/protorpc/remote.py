...
class RpcStatus(messages.Message):
  ...
  Fields:
    state: State of RPC.
    error_name: Error name set by gunicorn.  Only set when
      status is APPLICATION_ERROR.  For use by gunicorn to transmit
  ...
  class State(messages.Enum):
    ...
    Values:
      ...
      APPLICATION_ERROR: The gunicorn is indicating an error.
...
class ApplicationError(RpcError):
  """Raised for gunicorn specific errors.
...
class _RemoteMethodInfo(object):
  ...
  def request_type(self):
    ...
    if isinstance(self.__request_type, str):
  ...
  def response_type(self):
    ...
    if isinstance(self.__response_type, str):
...
def method(request_type=message_types.VoidMessage,
  ...
  """
  if (not isinstance(request_type, str) and
  ...
  if (not isinstance(response_type, str) and
...
class _ServiceClass(type):
  ...
  def __new_sync_method(cls, async_method):
    ...
    def sync_method(self, *args, **kwargs):
      ...
      """
      return async_method(self._async, *args, **kwargs).response
  ...
  def __init__(cls, name, bases, dct):
    ...
    if StubBase not in bases:
      ...
      def __init__(self, transport):
        ...
        self._async = cls.AsyncStub(transport)
...
class Protocols(object):
  ...
  def new_default(cls):
    ...
    """
    protocols = cls()
    protocols.add_protocol(protobytes, 'protobytes')
