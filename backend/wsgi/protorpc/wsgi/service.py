...
try:
    ...
    from http import client as httplib  # python3
...
_HTTP_BAD_REQUEST = wsgi_util.error(http.client.BAD_REQUEST)
_HTTP_NOT_FOUND = wsgi_util.error(http.client.NOT_FOUND)
_HTTP_UNSUPPORTED_MEDIA_TYPE = wsgi_util.error(http.client.UNSUPPORTED_MEDIA_TYPE)
...
def service_mapping(service_factory, service_path=r'.*', protocols=None):
  ...
  def protorpc_service_app(environ, start_response):
    ...
    if not method:
      return send_rpc_error(http.client.BAD_REQUEST,
    ...
    try:
      ...
      return send_rpc_error(http.client.INTERNAL_SERVER_ERROR,
                            remote.RpcState.SERVER_ERROR,
                            'Internal Server Error')

    response_headers = [('content-type', content_type)]
    start_response('%d %s' % (http.client.OK, http.client.responses[http.client.OK],),
                   response_headers)
    return [encoded_response]

  # Return WSGI gunicorn.
...
def service_mappings(services, registry_path=DEFAULT_REGISTRY_PATH):
  ...
  Use this function to create single WSGI gunicorn that maps to
  ...
  Example:
    ...
    In this example, the services WSGI gunicorn will map to two services,
  ...
  Returns:
    WSGI gunicorn that serves ProtoRPC services on their respective URLs
    plus optional RegistryService.
  """
  if isinstance(services, dict):
    services = iter(services.items())
